...
class PDM_Bulk_Alt_Scanner {
     ...
     */
    private function add_title_to_image_tag($img_tag, $title_text) {
        // Escape the title text for HTML attribute
        $escaped_title = esc_attr($title_text);
        
        // Check if title attribute already exists (even if empty)
        if (preg_match('/title\s*=\s*["\'][^"\']*["\']?/i', $img_tag)) {
            // Replace existing title attribute (including empty ones)
            $new_tag = preg_replace('/title\s*=\s*["\'][^"\']*["\']?/i', 'title="' . $escaped_title . '"', $img_tag);
        } else {
            // Add title attribute - try to place it after alt for better readability
            if (preg_match('/<img([^>]*)(alt\s*=\s*["\'][^"\']*["\']?)([^>]*>)/i', $img_tag, $matches)) {
                $new_tag = '<img' . $matches[1] . $matches[2] . ' title="' . $escaped_title . '"' . $matches[3];
            } elseif (preg_match('/<img([^>]*)(src\s*=\s*["\'][^"\']+["\']?)([^>]*>)/i', $img_tag, $matches)) {
                // If no alt, place after src
                $new_tag = '<img' . $matches[1] . $matches[2] . ' title="' . $escaped_title . '"' . $matches[3];
            } else {
                // Fallback: add right after <img
                $new_tag = str_replace('<img', '<img title="' . $escaped_title . '"', $img_tag);
            }
        }
    ...
    private function parse_image_tag($img_tag) {
        ...
        // Check if title attribute exists and get its value
        $has_meaningful_title = false;
        $title_value = '';
        
        if (preg_match('/title\s*=\s*["\']([^"\']*)["\']?/i', $img_tag, $title_matches)) {
            $title_value = trim($title_matches[1]);
            $has_meaningful_title = !empty($title_value);
        }
        ...
        return array(
            ...
            'alt_value' => $alt_value,
            'has_title' => $has_meaningful_title,
    ...
    private function parse_divi_image_shortcode($shortcode) {
        ...
        // Extract title_text attribute (Divi uses title_text for image titles)
        if (preg_match('/title_text=["\']([^"\']*)["\']?/i', $shortcode, $title_matches)) {
    ...
    /**
     * Add title_text attribute to Divi shortcode
     */
    private function add_title_to_divi_shortcode($shortcode, $title_text) {
        $escaped_title = esc_attr($title_text);
        
        // Check if title_text attribute already exists
        if (preg_match('/title_text=["\'][^"\']*["\']?/i', $shortcode)) {
            // Replace existing title_text attribute
            $new_shortcode = preg_replace('/title_text=["\'][^"\']*["\']?/i', 'title_text="' . $escaped_title . '"', $shortcode);
        } else {
            // Add title_text attribute - place it after alt for better readability
            if (preg_match('/(alt=["\'][^"\']*["\']?)/', $shortcode, $matches)) {
                $new_shortcode = str_replace($matches[1], $matches[1] . ' title_text="' . $escaped_title . '"', $shortcode);
            } elseif (preg_match('/(src=["\'][^"\']+["\']?)/', $shortcode, $matches)) {
                // If no alt, place after src
                $new_shortcode = str_replace($matches[1], $matches[1] . ' title_text="' . $escaped_title . '"', $shortcode);
            } else {
                // Fallback: add before closing bracket
                $new_shortcode = str_replace(']', ' title_text="' . $escaped_title . '"]', $shortcode);
            }
        }
